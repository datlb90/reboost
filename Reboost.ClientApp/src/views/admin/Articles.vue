  ...
  <div class="container">
    ...
    <el-table ref="filterTable" size="mini" :data="articlesList" stripe style="width: 100%;">
      ...
      <el-table-column :label="messageTranslates('adminArticles', 'articleAuthor')">
        <template slot-scope="scope">
          <span style="word-break: break-word">{{ scope.row.authorId }} </span>
        </template>
      </el-table-column>
      <el-table-column
      ...
      <el-table-column :label="messageTranslates('adminArticles', 'articleOriginalUrl')">
        <template slot-scope="scope">
          <a :href="scope.row.originalUrl" target="_blank" style="word-break: break-word">{{ scope.row.originalUrl }} </a>
        </template>
      </el-table-column>
