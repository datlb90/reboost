          ...
          <el-form-item label="Apply to Become">
            <div style="display: flex; transform: translateY(40%);">
              <el-checkbox-group v-model="formRegister.applyToBecome.IELTS" style="margin-right: 20px;">
                <el-checkbox label="IELTS Rater" name="type" />
              </el-checkbox-group>
              <el-checkbox-group v-model="formRegister.applyToBecome.TOEFL">
                <el-checkbox label="TOEFL Rater" name="type" />
              </el-checkbox-group>
            </div>
          </el-form-item>
  ...
  methods: {
    ...
    loadDetail(id) {
      ...
      raterService.getById(id).then(rs => {
        ...
        // Apply to
        if (rs.applyTo) {
          const items = rs.applyTo.split(',')
          for (const key in this.formRegister.applyToBecome) {
            const exist = items.find(i => i == key)
            if (exist) {
    ...
    onSubmit(formName, createOrUpdate) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          const _applyTo = []
          for (const key in data.applyToBecome) {
