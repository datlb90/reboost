<template>
  ...
  >
      ...
      >
        ...
        <el-form-item id="questionPart" prop="part" size="mini" label="">
          ...
          <div v-if="form.test && form.task===3">
            <el-form-item prop="" size="mini" :label="messageTranslates('addEditQuestion', 'chart')">
              <el-upload
                action=""
                :on-preview="handlePreview"
    ...
    <span slot="footer" class="dialog-footer">
      <el-button size="mini" @click="dialogVisible = false">{{ constantTranslate('DISPUTE_STATUS', 'cancel') }}</el-button>
      <el-button v-if="isLearnerContributed" type="success" size="mini" @click="approveQuestion">{{ constantTranslate('DISPUTE_STATUS', 'approve') }}</el-button>
      <el-button size="mini" type="primary" @click="submit">{{ constantTranslate('DISPUTE_STATUS', 'submit') }}</el-button>
    </span>
